...
int main()
{
  ...
  // place a Great Hall card in the current players hand
  pre.hand[current][handPos] = great_hall;

  // get a baseline count of Great Hall cards in the hand
  for (int i = 0; i < pre.handCount[current]; i++)
  {
    if (pre.hand[current][i] == great_hall)
      preCount++;
  }
  ...
  // TEST #1 - Great Hall card played and discarded
  cardEffect(great_hall, choice1, choice2, choice3, &post, handPos, &bonus);

  // get current count of Great Hall cards after playing the card
  for (int i = 0; i < post.handCount[current]; i++)
  {
    if (post.hand[current][i] == great_hall)
      postCount++;
  }

  // print results of the test
  printf("Great Hall: Discarded Played Card & Net Gain +0 Cards\n");
  printf("%2d.) Great Hall Count = %2d, Expected = %2d: ", testNum, postCount, preCount - 1);
  ...
  // TEST #3 - +1 Action
  // print results of the test comparing current and expected hand counts
  printf("%2d.)           Action = %2d, Expected = %2d: ", testNum, post.numActions, pre.numActions + 1);
